    ...
    for iter_ in range(num_batches):
        ...
        for a in range(8):
            for b in range(6):
                row = patch_size * a
                col = patch_size * b
                patch_fake = pred[:, :, row:row+patch_size, col:col+patch_size]
        ...
        for a in range(8):
            for b in range(6):
                ...
                col = patch_size * b
                patch_fake = pred[:, :, row:row+patch_size, col:col+patch_size]
                patch_real = target[:, :, row:row+patch_size, col:col+patch_size]
...
def validate(val_loader, model, discriminator, criterion_L1, criterion_MSE, 
    ...
    for i in range(num_batches):
        ...
        if i==0:
            save_image(model, input, target, train_iter, "val")
...
model = ResNet(layers=101, output_size=(320, 240), pretrained=True)
...
if os.path.exists("./PatchGAN.pth"):
    checkpoint = torch.load("./PatchGAN.pth")
